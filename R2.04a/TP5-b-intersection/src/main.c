...
u32 is_intersecting_brick(s16 ball_x, s16 ball_y, s16 brick_x, s16 brick_y);

static u32 is_bouncing_on_brick(Brick * bricks, u32 index, s16 ball_x, s16 ball_y, s16 * ptr_dir_x, s16 * ptr_dir_y) {
	Brick * brick = &(bricks[index]);
	
	if (   (brick -> visible == 1)
	    && (is_intersecting_brick(ball_x, ball_y, brick -> x, brick -> y) == 1)) {
		brick -> visible = 0;
...
static u32 check_collisions(Brick * bricks, s16 ball_x, s16 ball_y, s16 * ptr_dir_x, s16 * ptr_dir_y) {
	u32 brick_index;
	u32 i;
	u32 D1;
	...
	i = 0;
	brick_index = -1;
	while ((i < NB_BRICKS) && (brick_index == -1)) {
		D1 = is_bouncing_on_brick(bricks, i, ball_x, ball_y, ptr_dir_x, ptr_dir_y);
		if (D1 != 0) { brick_index = i; break; }
		i++;
	}
