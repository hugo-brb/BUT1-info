public class ListeChainee<TypeInfo extends Comparable<TypeInfo>>
     ...
     * @return true
     * @pre aucune
     * @post un nouvel élément a été inséré en queue de liste
     */

    @Override
    public void insereQueue(TypeInfo nouvelleInfo) {
        if (estVide()) {
            tete = new Cellule<>(nouvelleInfo);
        } else {
            Cellule<TypeInfo> cc = tete;
            while(cc.getCelluleSuivante()!=null) {
                cc = cc.getCelluleSuivante();
            }
            Cellule<TypeInfo> celluleQueue = new Cellule<>(nouvelleInfo);
            cc.setCelluleSuivante(celluleQueue);
        }
        nbCellules++;
